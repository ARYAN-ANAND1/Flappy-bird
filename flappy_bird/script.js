...
playerImage.src = 'player.png'; // You can change this to your image filename

var obstacleImage = new Image();
obstacleImage.src = 'obstacle.png'; // You can change this to your image filename
...
class Square{
    ...
    draw(){  
        ...
        if(playerImage.complete){
            context.drawImage(playerImage, this.vertex_x[0], this.vertex_y[0], this.side, this.side);
        } else {
            // Fallback to original square if image not loaded
...
class Pipe{
    ...
    draw(){
        if(obstacleImage.complete){
            // Draw image for top pipe
            context.drawImage(obstacleImage, this.coords[0][0], 0, this.width, this.coords[0][1]);
            // Draw image for bottom pipe
            context.drawImage(obstacleImage, this.coords[3][0], this.coords[3][1], this.width, stage.height-this.coords[2][1]);
        } else {
            // Fallback to original black rectangles if image not loaded
...
animate=()=>{
    ...
    if(started){
        ...
        context.textAlign='center';
        context.font='20px Tahoma';     
        context.fillText('Push any key to begin...', stage.width/2, stage.height/2);
    }}
}
animate();